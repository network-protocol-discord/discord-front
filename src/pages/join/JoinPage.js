  ...
  return (
    <AuthTemplate>
      <div style={{ marginTop: "10px"}}>
        ...
        <input id="id" type="text" value={username} onChange={(e) => setUsername(e.target.value)} onKeyDown={(e) => handleKeyPress(e)} required/>
        {error === '이미 존재하는 아이디입니다.' && <div className="warning">이미 사용중인 아이디 입니다.</div>}
        <label for="name" style={{marginTop: "20px"}}>사용자명</label>
        <input id="name" type="text" value={nickname} onChange={(e) => setNickname(e.target.value)} onKeyDown={(e) => handleKeyPress(e)} required/>
        {error === '이미 존재하는 닉네임입니다.' && <div className="warning">이미 사용중인 닉네임 입니다.</div>}
        <label for="email" style={{marginTop: "20px"}}>이메일</label>
        <input id="email" type="text" onChange={(e) => setEmail(e.target.value)} onKeyDown={(e) => handleKeyPress(e)} required/>
        <label for="pw" style={{marginTop: "20px"}}>비밀번호</label>
        <input id="pw" type="password" onChange={(e) => setPassword(e.target.value)} onKeyDown={(e) => handleKeyPress(e)} required/>
